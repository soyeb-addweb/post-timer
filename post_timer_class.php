...
class addweb_pt_post_timer
{
	...
	public function __construct()
	{
		...
		if (isset($_SERVER['REQUEST_URI'])) {
			// Unsanitize the value if needed.
			$addweb_pt_url = sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI'])); // Remove any slashes added by WordPress or PHP
		} else {
			$addweb_pt_url = ''; // Default value if REQUEST_URI is not set
		}

		$addweb_pt_querystr =  isset($_GET['post_type']) ? sanitize_text_field(wp_unslash($_GET['post_type'])) : ''; // Remove any slashes added by WordPress or PHP $_GET['post_type'];
		$addweb_pt_actionqry =  isset($_GET['action']) ? sanitize_text_field(wp_unslash($_GET['action'])) : '';
		$addweb_pt_postid = isset($_GET['post']) ? sanitize_text_field(wp_unslash($_GET['post'])) : '';
	...
	{
		add_options_page(__('Post Timer Settings', 'post-timer'), __('Post Timer', 'post-timer'), 'manage_options', $this->addweb_pt_plugin_slug, array($this, 'addweb_pt_timer_popup_options'));
	}
	...
	public function addweb_pt_enqueue_admin_scripts()
	{
		...
		if ('settings_page_' . $this->addweb_pt_plugin_slug == $screen->id) {
			wp_enqueue_script($this->addweb_pt_plugin_slug . '-admin-script', plugins_url('js/admin.js', __FILE__), array('jquery', 'wp-color-picker'), addweb_pt_post_timer::ADDWEB_PT_VERSION, 'false');
			wp_enqueue_media();
		}
	}
	...
	public function addweb_pt_add_action_links($links)
	{
		return array_merge(
			array(
				'settings' => '<a href="' . admin_url('options-general.php?page=' . $this->addweb_pt_plugin_slug) . '">' . esc_html_e('Settings', 'post-timer') . '</a>'
			),
			$links
		);
	}
	...
	public function addweb_pt_timer_popup_options()
	{
		if (! current_user_can('manage_options')) {
			wp_die(esc_html_e('You do not have sufficient permissions to access this page.', 'post-timer'));
		}
		...
		if (! empty($_POST) && check_admin_referer('addweb_pt_post_timer', 'save_timer_popup')) {
			...
			// } else {
			// 	add_option('addweb_pt_popup_active', $_POST['addweb_pt_popup_active'], "", 'no');
			// }

			if (isset($_POST['addweb_pt_popup_active'])) {
				// Sanitize the value to ensure it is safe to use
				$popup_active = sanitize_text_field(wp_unslash($_POST['addweb_pt_popup_active']));

				// Check if the option exists and update it
				if ($this->addweb_pt_popup_active !== false) {
					update_option('addweb_pt_popup_active', $popup_active);
				} else {
					add_option('addweb_pt_popup_active', $popup_active, "", 'no');
				}
			} else {
				// Handle the case where the $_POST['addweb_pt_popup_active'] is not set (optional)
				// For example, log an error, or set a default value
				$popup_active = ''; // Or any other default value you want to use
			}


			//add or update timer popup header color
			if (isset($_POST['addweb_pt_popup_color'])) {
				$popup_color = sanitize_text_field(wp_unslash($_POST['addweb_pt_popup_color']));
				//add or update timer popup header color
				if ($this->addweb_pt_popup_color !== false) {
					update_option('addweb_pt_popup_color', $popup_color);
				} else {
					add_option('addweb_pt_popup_color', $popup_color, "", 'no');
				}
			} else {
				$popup_color = '';
			}
			...
			//add or update timer popup 
			if (isset($_POST['addweb_pt_popup_place'])) {
				$popup_place = sanitize_text_field(wp_unslash($_POST['addweb_pt_popup_place']));
				//add or update timer popup header color
				if ($this->addweb_pt_popup_place !== false) {
					update_option('addweb_pt_popup_place', $popup_place);
				} else {
					add_option('addweb_pt_popup_place', $popup_place, "", 'no');
				}
			} else {
				$popup_place = '';
			}
			...
			if (isset($_POST['addweb_pt_popup_top_margin'])) {
				$popup_top_margin = sanitize_text_field(wp_unslash($_POST['addweb_pt_popup_place']));
				//add or update timer popup header place
				if ($this->addweb_pt_popup_top_margin !== false) {
					update_option('addweb_pt_popup_top_margin', $popup_top_margin);
				} else {
					add_option('addweb_pt_popup_top_margin', $popup_top_margin, "", 'no');
				}
			} else {
				$popup_top_margin = '';
			}

			if (isset($_POST['addweb_pt_popup_posts'])) {
				$popup_post = sanitize_text_field(wp_unslash($_POST['addweb_pt_popup_place']));
				if ($this->addweb_pt_popup_posts !== false) {
					update_option('addweb_pt_popup_posts', $popup_post);
				} else {
					add_option('addweb_pt_popup_posts', $popup_post, "", 'no');
				}
			} else {
				$popup_post = '';
			}
			wp_redirect(esc_url(admin_url('options-general.php?page=' . isset($_GET['page']) ? sanitize_text_field(wp_unslash($_GET['page'])) : 'post-timer' . '&updated=1')));
		}
?><div class="pt-wrap" style="max-width: 1000px; width:100%;">
			<div class="fa-plugin-setting">
				<ul>
					<li><a href="#pt-setting"><?php esc_html_e('Settings', 'post-timer'); ?></a></li>
					<li><a href="#pt-about"><?php esc_html_e('About Us', 'post-timer'); ?></a></li>
				</ul>
				<div id="pt-setting">
					<h2><?php esc_html_e('Timer Popup Settings', 'post-timer'); ?></h2>
					<form method="post" action="<?php echo esc_url(admin_url('options-general.php?page=' . sanitize_text_field(wp_unslash($_GET['page'])) . '&noheader=true')); ?>" enctype="multipart/form-data"><?php
							...
							<table class="form-table" width="100%">
								<tr>
									<th scope="row"></th>
									<td>
										<input type="checkbox" name="addweb_pt_popup_active" id="addweb_pt_popup_active" value="1" <?php if ($this->addweb_pt_popup_active)  echo 'checked="checked"';
																																	else ''; ?>>&nbsp;<label for="addweb_pt_popup_active"><strong><?php esc_html_e('Enable', 'post-timer'); ?></strong></label>
									</td>
								</tr>
								<tr>
									<th scope="row"><label for="addweb_pt_popup_color"><?php esc_html_e('Popup Color', 'post-timer'); ?></label></th>
									<td><input type="text" name="addweb_pt_popup_color" id="popup_color" maxlength="255" size="25" value="<?php echo ($this->addweb_pt_popup_color) ? esc_attr($this->addweb_pt_popup_color) : ''; ?>" class="color-picker" data-default-color="<?php echo esc_attr($this->addweb_pt_popup_color); ?>"></td>
								</tr>
								<tr>
									<th scope="row"><label for="addweb_pt_popup_place"><?php esc_html_e('Popup Place', 'post-timer'); ?></label></th>
									<td><select name="addweb_pt_popup_place" id="addweb_pt_popup_place">
											<?php foreach ($this->addweb_pt_get_popup_place() as $key => $value): ?>
												<option value="<?php echo esc_attr($key); ?>"
													<?php echo $key == $this->addweb_pt_popup_place ? 'selected="selected"' : ''; ?>>
								...
								<tr>
									<th scope="row"><label for="addweb_pt_popup_top_margin"><?php esc_html_e('Popup Top Margin', 'post-timer'); ?></label></th>
									<td><input type="number" name="addweb_pt_popup_top_margin" id="addweb_pt_popup_top_margin" maxlength="255" size="25" value="<?php echo esc_attr($this->addweb_pt_popup_top_margin); ?>">%<br>
										<small>Top margin is only included if popup place Left or Right is selected. Please enter numeric value.</small>
									</td>
								</tr>
								<tr>
									<th scope="row" colspan="2">Choose Where To Show Popup</th>
								</tr><?php
										...
										?><tr>
											<th scope="row"></th>
											<td><input type="checkbox" name="addweb_pt_popup_posts[]" id="<?php echo esc_attr($addweb_pt_post_type->name); ?>" value="<?php echo esc_attr($addweb_pt_post_type->name); ?>" <?php if (in_array($addweb_pt_post_type->name, $this->addweb_pt_popup_posts))  echo 'checked="checked"';
																																																							else ''; ?> /> <label for="<?php echo esc_attr($addweb_pt_post_type->name); ?>"><strong><?php echo esc_html($addweb_pt_post_type->label); ?></strong></label>
											</td>
										</tr><?php
											}
										}
							...
							<p class="submit">
								<input type="submit" name="Submit" class="button-primary" value="<?php esc_attr_e('Save Changes', 'post-timer') ?>" />
							</p>
						</div>
					</form>
				</div>
				<div id="pt-about">
					<div style="margin:0 auto;width:54%;">
						<a href="http://www.addwebsolution.com" style="outline: hidden;" target="_blank"><img src="<?php echo esc_url(plugins_url('/images/addweb-logo.png', __FILE__)); ?>" alt="AddwebSolution" height=60px></a>
					</div><?php
					...
					<div class="advertise">
						...
						<div class="ad-content"><?php
								...
								<div class="ad-detail">
									<a href="<?php echo esc_html("https://wordpress.org/plugins/" . $slug); ?>" target="_blank"><img src="<?php echo esc_url(plugins_url('images/', __FILE__) . $slug); ?>.svg"></a>..
									<a href="<?php echo esc_html("https://wordpress.org/plugins/" . $slug); ?>" class="ad-link" target="_blank"><?php echo esc_attr($name); ?></a>
								</div><?php
												} ?>
						</div>
					</div>
			...
			{
				wp_enqueue_script($this->addweb_pt_plugin_slug . '-modernizr-script', plugins_url('js/modernizr.custom.js', __FILE__), array(), self::ADDWEB_PT_VERSION, 'false');
			}
			...
			public function addweb_pt_get_timer_popup()
			{
				...
				// Validate the property to avoid warnings or errors.
				if (!isset($this->addweb_pt_popup_place)) {
					return; // Exit if the property is not set.
				}
				...
				// Check the placement and generate the appropriate HTML.
				if ($this->addweb_pt_popup_place != 'top-left' && $this->addweb_pt_popup_place != 'top-right') {
					$addweb_pt_popup_html .= '<div class="popup-header">';
				...
				// Close wrapper divs.
				$addweb_pt_popup_html .= '</div>'; // .popup-wrap
				$addweb_pt_popup_html .= '</div>'; // .addweb-pt-timer-popup

				// Sanitize and output the HTML safely.
				$allowed_html = array(
					'div' => array(
						'class' => array(),
						'onclick' => array(),
			...
			.addweb-pt-timer-popup .popup-header {
				...
				?>background-color: <?php echo esc_attr($this->addweb_pt_popup_color); ?>;
				<?php
					} else {
			...
			.addweb-pt-timer-popup-left {
				...
				?>top: <?php echo esc_attr($this->addweb_pt_popup_top_margin); ?>%;
				<?php
						} else {
				?>top: 25%;
				<?php
